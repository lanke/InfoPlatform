...
class UsersListJson(BaseDatatableView):
    model = User
    columns = ['id', 'username','profile.phoneNumber','profile.department','email','profile.gender','profile.birthday','id']
    order_columns = ['id']
...
class UnfriendListJson(BaseDatatableView):
    ...
    def get_initial_queryset(self):
        ...
        #User.objects.filter(loginUser__user=user)
        #friendList=  FriendInfo.objects.filter(user=user)
        return User.objects.exclude(id__in=FriendInfo.objects.filter(user=user).values_list("userFriend_id",flat=True)).exclude(id=user.id)  
    def filter_queryset(self, qs):
        sSearch = self.request.GET.get('sSearch', None)
        if sSearch:
            qs = qs.filter(Q(username__istartswith=sSearch) | Q(email__istartswith=sSearch))
